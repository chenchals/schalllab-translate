...
function [dat,sev] = readSevCompare(sessionDir, nChannels, nSpikes)
...
%     nSpikes = 10000;
    ...
    for ch = 1:nChannels
        ...
        tic;
        % Use SEV2Mat
        data = SEV2mat(fullFilename,'CHANNEL',ch);
        sev{ch} = data.Wav1.data(1:nSpikes);
        clearvars data
        toc;
        tic;
        % Read directly
        ...
        % read header
        fseek(fid,40,'bof');
        dat{ch} = fread(fid,nSpikes,['*' header.dForm])';
        fclose(fid);
        toc;
    end
end
